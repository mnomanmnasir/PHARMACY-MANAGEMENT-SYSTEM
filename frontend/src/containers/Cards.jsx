...
const Card = ({ Toggle }) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const productCount = inventoriesResponse.data.length;
            const inventoriesCount = inventoriesResponse.data.length;

            setCounts({ sales: salesCount, purchases: inventoriesCount, products: productCount });
            setLoading(false);
        } catch (error) {
                    ...
                    <div className="row g-1 my-2">
                        <div className="col-md-3">
                            <div className="p-3 bg-green d-flex justify-content-around align-items-center shadow-sm rounded" style={{ backgroundColor: '#28a745' }}>
                                <h3 className='fs-0' style={{ color: 'white' }}>{counts.products}</h3>
                                <div>
